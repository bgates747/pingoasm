...
Status: historical execution plan. The Hecker campaign produced the accepted
exact-span, subdivided-affine, and incremental-depth checkpoints; unresolved
follow-ups now live in `pingo-renderer-optimization-todo.md`. Unchecked boxes
below describe the original staged plan and are not the current project queue.

Historical checkpoint: exact-output candidate `agon-vdp:b87c95e` qualified in
the emulator and on physical hardware. Two complete hardware runs and the
author's visual review passed. The campaign subsequently continued through
the checkpoints named above; this records its then-current state.
...
1.1 Completed — Finish hardware qualification of the current upstream-derived math
tranche before beginning Hecker work.

1.2 Completed — Commit and tag that exact source as the new known-working point of
departure. Archive its ordinary and diagnostic firmware binaries outside Git,
with source commit and SHA-256 identities.

1.3 Completed — Create one `experiment/hecker-rasterizer` branch. Use one small commit
per independently reversible experiment instead of creating a branch for
every variation.

1.4 Completed — Do not import either historical `hecker` branch wholesale. Treat
...
1.5 Completed — Preserve the current VDU protocol, object model, coordinate
...
1.6 Completed — Use only the fifteen fixtures already present on the SD card for
...
1.7 Completed — Do not alter the project emulator snapshot or commit emulator changes
...
2.1 Uncompleted historical item — Record a manifest containing the SHA-256 and size of every
`/pingo/<fixture>/benchmark.bin`, texture, and the card's `autoexec.txt`.

2.2 Completed — Configure `autoexec.txt` as one synchronous, fixed-order chain:
...
2.3 Completed — Teach the capture parser the two persistent control streams. The
...
2.4 Completed — Expect 36 records from each of eight stationary fixtures, 73 from each
...
3.1 Completed — Run the known-working ordinary firmware through the complete hardware
...
3.2 Uncompleted historical item — Run the exact known-working firmware through every fixture in the
...
3.3 Uncompleted historical item — Store baseline image hashes plus the raw target bytes. Hash equality is
...
3.4 Uncompleted historical item — Preserve, outside the repositories, a run directory containing source
...
4.1 Completed — Reject a candidate immediately if the scope diff contains unrelated
VDU, object-model, coordinate, texture-format, callback, or deployment
changes.

4.2 Completed — Require ordinary and diagnostic native tests to pass.

4.3 Completed — Require ordinary and diagnostic PlatformIO firmware builds to pass.

4.4 Completed — Run the headless emulator before flashing hardware. For a
semantics-preserving optimization, require exact selected-frame and
final-target equality across all fifteen fixtures.

4.5 Completed — If a candidate intentionally changes pixel coverage rules, preserve
...
4.6 Completed — Flash only a candidate that passes the emulator gate. Stop any serial
...
4.7 Completed — Fail the hardware run on a missing or malformed record, unexpected
...
4.8 Uncompleted historical item — Save a failed candidate and its evidence, restore the known-working
...
5.1 Completed — First extract testable raster primitives and characterize current
...
5.2 Uncompleted historical item — Add incremental interpolation inside the existing bounding-box
...
5.3 Completed — Evaluate replacing the two per-fragment perspective divisions with one
...
5.4 Completed — Introduce scanline span bounds as a coverage accelerator while keeping
...
5.5 Uncompleted historical item — Add top-left/shared-edge ownership rules only as a separate candidate.
...
5.6 Uncompleted historical item — Only after the floating-point span reference is correct, evaluate the
...
5.7 Uncompleted historical item — Keep geometric frustum clipping separate. The card's moving scenes
...
5.8 Completed — Run diagnostic firmware only to attribute a change. State absolute
frame-time and FPS improvements only from ordinary firmware.

## 6. Hardware test cadence

6.1 Uncompleted historical item — During rapid iteration, use the first four programs in the fixed chain
...
6.2 Completed — Run all 1,447 records once for every candidate that survives the quick
gate.

6.3 Completed — Repeat the full chain for a candidate proposed for retention. The plan
...
6.4 Uncompleted historical item — Retain an exploratory candidate when it produces a repeatable,
fixture-specific signal worth investigating and does not regress another
fixture by more than normal baseline variance.

6.5 Completed — Propose promotion only when correctness gates pass and the candidate
...
7.1 Completed — Before sleep, verify that the hardware is powered, the card is in the
...
7.2 Completed — Bound the number of candidate flashes and retries. A failed upload may
...
7.3 Completed — Never rewrite the SD card, change MOS, or change application binaries
during the unattended run.

7.4 Completed — Leave a machine-readable ledger showing every candidate attempted,
...
7.5 Completed — End with the known-good firmware unless one candidate passed every
...
8.1 Uncompleted historical item — The card contains only RGBA2222 fixtures. No Hecker candidate may be
declared legacy-compatible until an RGBA8888 regression fixture is tested
later.

8.2 Completed — Cube, EarthUV, Jet, and Airliner now provide controlled camera motion
...
8.3 Completed — Serial timing confirms completion and performance; it does not prove
...
8.4 Uncompleted historical item — Existing application records do not carry an explicit fixture ID.
...
9.1 Completed — A concise ledger of accepted and rejected experiments.

9.2 Completed — Raw and parsed emulator and hardware evidence for every retained or
important rejected candidate.

9.3 Completed — Separate commits for each reversible code experiment, with no emulator
snapshot commit.

9.4 Completed — A recommendation to retain, revise, or abandon the Hecker direction,
including which raster subphase changed and which fixtures benefited.

9.5 Completed — The exact firmware identity left on the hardware and the command to
