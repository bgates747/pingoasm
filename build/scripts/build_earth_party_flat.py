#!/usr/bin/env python3
"""Build the flat-vehicle, self-illuminated-star Earth Party sibling fixture."""
...
APP_ROOT = PROJECT_ROOT / "apps" / "earth-party-flat"
...
OUTPUT_FILENAME = "earth-party-flat.bin"
STARFIELD_INCLUDE = "starfield.inc"
POSE_HELPER_INCLUDE = "pose-cycle.inc"
EARTH_POSE_INCLUDE = "earth-spin-cycle.inc"
COMMON_POSE_HELPER = PROJECT_ROOT / "tests" / "apps" / "_common" / POSE_HELPER_INCLUDE
GENERATOR = "build/scripts/build_earth_party_flat.py"
...
FLAT_MODELS = frozenset(("jet", "airliner"))
PALETTE_RGBA2 = PROJECT_ROOT / "src" / "blender" / "colors64.rgba2"
PALETTE_PNG = PROJECT_ROOT / "src" / "blender" / "colors64.png"
PALETTE_SIZE = (8, 8)
...
def load_profile() -> dict[str, Any]:
    ...
    for model in models:
        ...
        names.append(name)
        shading = str(model.get("shading", "textured"))
        expected_shading = "flat_palette" if name in FLAT_MODELS else "textured"
        if shading != expected_shading:
            raise ValueError(
                f"{name} shading must be {expected_shading}, not {shading}"
            )
    ...
    if profile.get("name") != "earth-party-flat":
        raise ValueError("profile name must be earth-party-flat")
    for palette in (PALETTE_PNG, PALETTE_RGBA2):
        if not palette.is_file():
            raise ValueError(f"missing canonical palette: {palette.name}")
    ...
    if starfield.get("shading") != "flat_palette":
        raise ValueError("starfield shading must be flat_palette")
    if starfield.get("illumination") != "self":
        raise ValueError("starfield illumination must be self")
...
def build() -> Path:
    ...
    from flat_palette import (
        convert_obj_model_data,
        validate_flat_palette_asm,
        verify_palette_image,
    )
    ...
    temporary_root = Path(
        tempfile.mkdtemp(prefix=".earth-party-flat-build-", dir=APP_ROOT)
    )
    ...
    try:
        ...
        palette_digest = hashlib.sha256(PALETTE_RGBA2.read_bytes()).hexdigest()
        if palette_digest != CANONICAL_PALETTE_SHA256:
            raise ValueError(
                "canonical RGBA2222 palette has unexpected SHA-256: "
                f"{palette_digest}"
            )
        textures: list[Path] = []
        for model in profile["models"]:
            ...
            with Image.open(texture_source) as image:
                ...
                if name not in FLAT_MODELS:
                    img_to_rgba2(image, texture_target)
            if name in FLAT_MODELS:
                shutil.copy2(PALETTE_RGBA2, texture_target)
            textures.append(texture_target)

            model_source = project_path(str(model["model_obj_source"]))
            if name in FLAT_MODELS:
                model_data, _ = convert_obj_model_data(
                    model_source,
                    texture_source,
                )
                output_texture_size = PALETTE_SIZE
                provenance = (
                    f"{model_source.relative_to(PROJECT_ROOT).as_posix()}; "
                    f"{texture_source.relative_to(PROJECT_ROOT).as_posix()}; "
                    "src/blender/colors64.rgba2"
                )
            else:
                model_data = parse_obj_file(model_source)
                output_texture_size = expected_size
                provenance = model_source.relative_to(PROJECT_ROOT).as_posix()
            write_data(
                model_source.stem,
                *model_data,
                ...
                output_texture_size,
                symbol_prefix=name,
                authoritative_input=provenance,
                generator="build/scripts/build_earth_party_flat.py",
            )
            if name in FLAT_MODELS:
                validate_flat_palette_asm(
        ...
        # The pose-cycle machinery is shared build material, but both the
        # helper and generated data are copied into this app's src directory
        # so the resulting assembly source remains self-contained.
        write_generated_snapshot(
            COMMON_POSE_HELPER,
            generated_source / POSE_HELPER_INCLUDE,
            generator=GENERATOR,
            source_label="tests/apps/_common/pose-cycle.inc",
        )
        write_pose_cycle_include(
            ...
            generator=GENERATOR,
            authoritative_input="apps/earth-party-flat/profile.json",
        )
