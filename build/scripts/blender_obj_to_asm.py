...
def write_data(
    ...
    *,
    ...
    symbol_prefix = symbol_prefix or base_filename
    source_line = (
    ...
    with open(tgt_filepath, 'w') as file:
        file.write(
            ...
            '; Generated by: build/scripts/blender_obj_to_asm.py\n'
            f'{source_line}'
            '; Edit the generator or its authoritative inputs instead.\n'
            '; =============================================================================\n\n'
        )
        file.write(f'{symbol_prefix}_indices_n: equ {len(faces) * 3}\n')
        file.write(f'{symbol_prefix}_vertices_n: equ {len(vertices)}\n')
        file.write(f'{symbol_prefix}_uvs_n: equ {len(texture_coords)}\n')
        file.write(f'{symbol_prefix}_normals_n: equ {len(normals)}\n')

        with open(uv_texture_rgba8, 'rb') as img_file:
            img_data = img_file.read()
            img_width, img_height = img_size
            filesize = len(img_data)
            file.write(f'{symbol_prefix}_texture_width: equ {img_width}\n')
            file.write(f'{symbol_prefix}_texture_height: equ {img_height}\n')
            file.write(f'{symbol_prefix}_texture_size: equ {filesize}\n')
        ...
        file.write(f'\n; -- VERTICES --\n')
        file.write(f'{symbol_prefix}_vertices:\n')
        ...
        file.write(f'\n; -- FACE VERTEX INDICES --\n')
        file.write(f'{symbol_prefix}_vertex_indices:\n')
        for item in faces:
            file.write(f'\tdw {", ".join(map(str, item))}\n')
        ...
        file.write(f'{symbol_prefix}_uv_indices:\n')
        for item in texture_vertex_indices:
            file.write(f'\tdw {", ".join(map(str, item))}\n')

        # Write the normals
        file.write(f'\n; -- NORMALS --\n')
        file.write(f'{symbol_prefix}_normals:\n')
        for item in normals:
            item = [round(coord * 32767) for coord in item]
            file.write(f'\tdw {", ".join(map(str, item))}\n')
        ...
        file.write(f'\n; -- NORMAL INDICES --\n')
        file.write(f'{symbol_prefix}_normal_indices:\n')
        for item in normal_indices:
            file.write(f'\tdw {", ".join(map(str, item))}\n')

        file.write(f'\n{symbol_prefix}_texture: db "{os.path.basename(uv_texture_rgba8)}",0\n')
