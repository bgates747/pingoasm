...
`earth-party-flat-local` is a rendering-policy sibling of
`earth-party-local`. It combines the same asynchronous moveair Jet and
...
Nothing in the simulation, controls, camera tracking, object placement,
velocity, scale, viewport, or asynchronous render cadence changes from
`earth-party-local`.

Only these rendering policies differ:
...
`profile.json` names all authoritative OBJ/PNG inputs and deliberately shares
the tracked star-catalogue snapshot with `earth-party-local`. The
...
```bash
.venv/bin/python build/scripts/build_earth_party_flat_local.py
```
...
apps/earth-party-flat-local/tgt/*.rgba2
```
...
```bash
.venv/bin/python build/scripts/deploy.py hardware earth-party-flat-local
```
