...
Assembly files
--------------
- app.asm: the main application file, links to all the other include files.
- app.inc: macros and functons specific to this application.
- input.inc: handles user keyboard input.
- model.inc: contains the 3D vertex and texture data for the 3D object to render.

Image files
-----------
- the various blenderaxes files are what provide the texturing for the 3D object. It is 34x34 pixels in dimension. The .xcf file is the original GIMP file containing various layers from which the exported .png image was derived. It uses the Agon 64 color palette. The .rgba2 and .rgba8 files are conversions of the .png image into the native Agon bitmap formats. The .rgba8 file is the one being uploaded by default, but this can be changed in the source code if desired.
...
Editing 3D object
-----------------
- model.inc is the only file which needs to be changed to modify the mesh and/or texture files and uv mappings.
